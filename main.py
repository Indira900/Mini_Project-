...
with app.app_context():
    ...
    if not User.query.first():
        ...
        # Bangalore Clinics
        bng_clinic1 = Clinic(name="Nova IVF Fertility", city="Bangalore", state="KA")
        bng_clinic2 = Clinic(name="Oasis Fertility", city="Bangalore", state="KA")
        bng_clinic3 = Clinic(name="Iswarya Fertility Centre", city="Bangalore", state="KA")
        bng_clinic4 = Clinic(name="Gaudium IVF Centre", city="Bangalore", state="KA")
        bng_clinic5 = Clinic(name="Indira IVF", city="Bangalore", state="KA")
...
def ivf_predictor():
    user = User.query.get(session['user_id'])
    patient_data = PatientData.query.filter_by(user_id=user.id).first()
    return render_template('ivf_predictor.html', user=user, patient_data=patient_data)
...
def wellness_data():
    ...
    data = {
        ...
        'detected_emotions': [log.detected_emotion for log in reversed(logs)] # Add detected emotions
    }
...
def predict_ivf():
    if not ivf_model:
        return jsonify({"error": "IVF prediction model not loaded."}), 503

    data = request.json
    app.logger.debug(f"Received /predict_ivf request data: {data}")
    try:
        features = np.array([[data[k] for k in ["Age", "BMI", "AMH", "FSH", "LH", 
                                                "Previous_IVF_Attempts", "Stress_Level", 
        ...
        # The probability of success is the probability of class '1'
        success_chance = probability[1] * 100

        app.logger.debug(f"Prediction: {prediction}, Probability: {success_chance:.2f}%")
        return jsonify({
            "prediction_text": "Likely Success" if prediction == 1 else "Low Chance",
            "success_probability": round(success_chance, 2)
        })
